...
// Ensure that this environment variable is a string
...
export const verifyUserByJWT: RequestHandler = asyncHandler(
  async (req: Request, _: Response, next: NextFunction) => {
    try {
      const token =
        req.cookies?.accessToken ||
        req.header("auth-token")?.replace("Bearer ", "");
      if (!token) {
...
export const verifyAdminByJwt: RequestHandler = asyncHandler(
  async (req: Request, _: Response, next: NextFunction) => {
    try {
      const token =
        req.cookies?.accessToken ||
